...
The sync processor uses `syncPosts` queries (not `listPosts`). These are generated by AppSync when conflict resolution is enabled on your backend (which it is if you used DataStore). You can find these queries in your auto-generated `src/graphql/queries.js` file. The key differences:
...
**Fallback when `sync*` queries are not generated:** If your AppSync schema does not have conflict resolution enabled, the `sync*` queries will not be generated. Check your `src/graphql/queries.js` — if you see `syncPosts`, `syncComments`, etc., conflict resolution is active. If not, fall back to `list*` queries. Delta sync is not possible without `sync*` queries — every sync is a full download — but the architecture otherwise works the same.
