...
**GraphQL operations used on this page** (`CREATE_POST`, `UPDATE_POST`, `DELETE_POST`, `GET_POST`, `LIST_POSTS`, and the `POST_DETAILS_FRAGMENT` fragment) are defined on the [Set up Apollo Client](/[platform]/build-a-backend/data/migrate-from-datastore/set-up-apollo/) page. Import them as needed:
